...
class Response
{
	...
	public function __construct(?string $content_type, string $data, ?int $code = null)
	{
	...
	public function checkException(): void
	{
		if (!$this->isSuccess())
		{
			throw new ApiException(
				$this->error['detail'] ?? (is_string($this->error['error'] ?? null) ? $this->error['error'] : 'API Error'),
				$this->error['code'] ?? 0
			);
		}
	}
